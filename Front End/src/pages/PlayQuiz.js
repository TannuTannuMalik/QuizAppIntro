...
const PlayQuiz = () => {
  ...
  useEffect(() => {
    const fetchQuiz = async () => {
      try {
        const response = await fetch(`http://localhost:8082/api/quizzes/play/${quizId}`, {
          method: "GET",
          credentials: "include",
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8082/api/quizzes/play/${quizId}/question/${currentQuestionIndex}/submit`,
        {
          method: "POST",
  ...
  const handleNextQuestion = async () => {
    ...
    if (nextIndex < totalQuestions) {
      try {
        const response = await fetch(
          `http://localhost:8082/api/quizzes/play/${quizId}/question/${nextIndex}`,
          {
            method: "GET",
            credentials: "include",
          }
      ...
      try {
        const response = await fetch(`http://localhost:8082/api/quizzes/play/${quizId}/complete`, {
          method: "GET",
          credentials: "include",
        });
